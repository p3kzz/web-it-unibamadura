...
@section('page-subtitle', 'Kelola informasi kontak institusi')
...
@section('content')
    ...
    <div class="mb-6">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div x-data="{ open: false }" class="relative">
                <button @click="open = !open" @click.away="open = false"
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <span class="text-sm font-semibold text-gray-700">{{ $type ? ucfirst($type) : 'Semua Tipe' }}</span>
                    </div>
                ...
                <div x-show="open" x-cloak x-transition:enter="transition ease-out duration-200"
                    ...
                    <a href="{{ route('admin.contact.index', array_filter(['search' => $search])) }}"
                        class="flex items-center gap-3 px-4 py-2.5 hover:bg-blue-50 transition-colors duration-150 {{ !$type ? 'bg-blue-50' : '' }}">
                        ...
                        <div class="flex-1">
                            <p class="text-sm font-semibold text-gray-800">Semua Tipe</p>
                        </div>
                    </a>

                    @foreach (['email', 'phone', 'address'] as $t)
                        <a href="{{ route('admin.contact.index', array_filter(['type' => $t, 'search' => $search])) }}"
                            class="flex items-center gap-3 px-4 py-2.5 hover:bg-blue-50 transition-colors duration-150 {{ $type === $t ? 'bg-blue-50' : '' }}">
                            <div class="flex-shrink-0">
                                @if ($type === $t)
                                    <div class="w-5 h-5 bg-blue-700 rounded-full flex items-center justify-center">
                                        <svg class="w-3 h-3 text-white" fill="none" stroke="currentColor"
            ...
            <button @click="$dispatch('open-create-contact')"
                ...
                <span class="font-semibold">Tambah Kontak</span>
            </button>
        </div>
    </div>

    <div id="ajax-table-container">
        @include('admin.pages.contact.partials.table')
    </div>

    @include('admin.pages.contact.partials.form-create')

    @include('admin.pages.contact.partials.form-edit')

    @include('admin.pages.contact.partials.form-show')
