    ...
    <div @click.away="open = false" x-transition:enter="transition ease-out duration-300"
        ...
        <div class="bg-uniba-blue px-6 py-4 flex items-center justify-between sticky top-0">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h3 class="text-xl font-bold text-white">Tambah Pilar Transformasi</h3>
        ...
        <form method="POST" action="{{ route('admin.tentang.pilar-transformasi.store') }}" class="p-6">
            @csrf
            ...
            <div class="space-y-5">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="title"
                            class="w-full border-2 border-gray-300 rounded-lg px-4 py-2.5 focus:border-uniba-blue focus:ring-2 focus:ring-uniba-blue focus:ring-opacity-20 transition-all duration-200 outline-none"
                            value="{{ old('title') }}" placeholder="Masukkan judul pilar" required>
                        @error('title')
                    ...
                    <div>
                        ...
                        <input type="text" name="subtitle"
                            class="w-full border-2 border-gray-300 rounded-lg px-4 py-2.5 focus:border-uniba-blue focus:ring-2 focus:ring-uniba-blue focus:ring-opacity-20 transition-all duration-200 outline-none"
                            value="{{ old('subtitle') }}" placeholder="Masukkan subtitle (opsional)">
                        @error('subtitle')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div>
                    ...
                    <textarea name="description" rows="4"
                        ...
                        placeholder="Masukkan deskripsi pilar transformasi...">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>

                <div x-data="{ components: [] }">
                        ...
                        <template x-for="(component, index) in components" :key="index">
                            <div class="flex gap-2">
                                <input type="text" :name="'key_components[' + index + ']'"
                                    x-model="components[index]"
                                    class="flex-1 border-2 border-gray-300 rounded-lg px-4 py-2 focus:border-uniba-blue focus:ring-2 focus:ring-uniba-blue focus:ring-opacity-20 transition-all duration-200 outline-none"
                                    placeholder="Masukkan komponen">
                                <button type="button" @click="components.splice(index, 1)"
                    ...
                    <button type="button" @click="components.push('')"
