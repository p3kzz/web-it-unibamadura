...
@section('title', 'Tinjauan Manajemen - UPT TIK UNIBA Madura')
        ...
        <div class="container mx-auto px-4">
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-4xl mx-auto">
                <div class="bg-white rounded-2xl p-4 md:p-6 shadow-lg text-center border border-gray-100">
                    <div class="text-2xl md:text-3xl font-bold text-uniba-blue">{{ $tinjauan->total() }}</div>
                    <div class="text-xs md:text-sm text-gray-500 font-medium">Total Tinjauan Manajemen</div>
                </div>
                <div class="bg-white rounded-2xl p-4 md:p-6 shadow-lg text-center border border-gray-100">
                    <div class="text-2xl md:text-3xl font-bold text-emerald-600">{{ $tinjauan->count() }}</div>
                    <div class="text-xs md:text-sm text-gray-500 font-medium">Ditampilkan Saat Ini</div>
                </div>
                <div class="bg-white rounded-2xl p-4 md:p-6 shadow-lg text-center border border-gray-100">
                    <div class="text-2xl md:text-3xl font-bold text-amber-600">{{ $tinjauan->sum('sections_count') }}</div>
    ...
    <section class="py-14 bg-gray-50">
        <div class="container mx-auto px-4">
            <div class="grid md:grid-cols-2 xl:grid-cols-3 gap-6 lg:gap-8">
                @forelse ($tinjauan as $item)
                    <a href="{{ route('tinjauan-manajemen.show', $item->slug) }}"
