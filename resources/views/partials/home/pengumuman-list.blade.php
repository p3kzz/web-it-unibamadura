<div class="space-y-4">
    ...
    @foreach ($pengumuman as $item)
        <div
            class="bg-white p-4 border-l-4 border-{{ $item['badge_color'] }}-500 shadow-sm rounded-r-lg flex items-start hover:shadow-md transition-shadow duration-300 transform hover:translate-x-1">
            <div class="flex-1">
                <h4
                    class="font-bold text-gray-800 hover:text-{{ $item['badge_color'] }}-600 cursor-pointer transition-colors">
                    {{ $item['title'] }}
                </h4>
