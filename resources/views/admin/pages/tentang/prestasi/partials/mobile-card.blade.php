        ...
        <div x-data="{ open: false }" class="relative">
            <button type="button" @click="open = !open" @click.away="open = false"
                ...
                <div class="flex items-center gap-2">
                    ...
                    <div class="text-left">
                        <p class="text-xs text-gray-400 leading-none mb-0.5">Filter Tahun</p>
            ...
            <div x-show="open" @click.outside="open = false" x-cloak
                ...
                <a href="{{ route('admin.tentang.prestasi.index', array_filter(['search' => $search])) }}"
                    class="flex items-center gap-3 px-4 py-3 hover:bg-blue-50 transition-colors duration-150
                        {{ !$yearFilter ? 'bg-blue-50' : '' }}">
                    <div
                        class="flex-shrink-0 w-8 h-8 rounded-lg flex items-center justify-center
                                {{ !$yearFilter ? 'bg-uniba-blue' : 'bg-gray-100' }}">
                        <svg class="w-4 h-4 {{ !$yearFilter ? 'text-white' : 'text-gray-400' }}" fill="none"
                    ...
                    <div>
                        <p class="text-sm font-semibold {{ !$yearFilter ? 'text-uniba-blue' : 'text-gray-800' }}">
                            Semua Tahun
                        </p>
                ...
                @foreach ($years as $year)
                    <a href="{{ route('admin.tentang.prestasi.index', array_filter(['year' => $year, 'search' => $search])) }}"
                        class="flex items-center gap-3 px-4 py-3 hover:bg-blue-50 transition-colors duration-150
                            {{ $yearFilter == $year ? 'bg-blue-50' : '' }}">
                        <div
                            class="flex-shrink-0 w-8 h-8 rounded-lg flex items-center justify-center
                                    {{ $yearFilter == $year ? 'bg-uniba-blue' : 'bg-gray-100' }}">
                            <svg class="w-4 h-4 {{ $yearFilter == $year ? 'text-white' : 'text-gray-400' }}"
                        ...
                        <div class="flex-1 min-w-0">
                            <p
                                class="text-sm font-semibold truncate
                                    {{ $yearFilter == $year ? 'text-uniba-blue' : 'text-gray-800' }}">
                                {{ $year }}
                            </p>
                            <p class="text-xs text-gray-400">Tahun {{ $year }}</p>
                        </div>
                        @if ($yearFilter == $year)
        ...
        <div
            ...
            <div class="flex items-center justify-between px-4 py-3 border-b border-gray-100 bg-gray-50">
                <div class="flex items-center gap-2.5">
                    ...
                    <span
                        class="inline-flex items-center px-2.5 py-1 bg-blue-100 text-blue-700 text-xs font-semibold rounded-full">
                        {{ $item->year }}
                    </span>
                </div>
            ...
            <div class="p-4 space-y-3">
                ...
                <div>
                    <p class="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-1">Pemberi Penghargaan</p>
                    <p class="text-sm text-gray-700">{{ $item->organization }}</p>
                </div>
                @if ($item->description)
                    <div class="border-t border-gray-100"></div>
                    <div>
            ...
            <div class="grid grid-cols-3 gap-2 px-4 pb-4">
                <button @click="$dispatch('open-show-prestasi', {{ $item->toJson() }})"
                ...
                <button @click="$dispatch('open-edit-prestasi', {{ $item->toJson() }})"
                    class="inline-flex items-center justify-center gap-2 px-4 py-2.5
                ...
                <form method="POST" action="{{ route('admin.tentang.prestasi.destroy', $item) }}"
    ...
    @empty
        <div class="bg-white rounded-2xl shadow-md border border-gray-100 p-10 text-center">
            ...
            <p class="text-gray-400 text-sm mb-5 leading-relaxed">
                @if ($search)
                    Tidak ditemukan prestasi dengan kata kunci "<span
