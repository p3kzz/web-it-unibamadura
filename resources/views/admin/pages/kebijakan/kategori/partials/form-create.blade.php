<div x-data="{
    open: false
}" x-on:open-create-category.window="open = true" x-show="open" x-cloak
    ...
    <div @click.away="open = false" x-transition:enter="transition ease-out duration-300"
        ...
        <div class="bg-uniba-blue px-6 py-4 flex items-center justify-between">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h3 class="text-xl font-bold text-white">Tambah Kategori</h3>
                    <p class="text-blue-100 text-sm">Buat kategori kebijakan baru</p>
                </div>
            </div>
        ...
        <form method="POST" action="{{ route('admin.penjaminan.policy-category.store') }}" class="p-6">
            @csrf
            ...
            <div class="space-y-5">
                <div>
                    ...
                    <input type="text" name="name"
                        ...
                        placeholder="Contoh: Kebijakan Keamanan" value="{{ old('name') }}" required>
                    @error('name')
                ...
                {{-- <div>
                    ...
                    <input type="text" name="slug"
                        class="w-full border-2 border-gray-300 rounded-lg px-4 py-2.5 focus:border-uniba-blue focus:ring-2 focus:ring-uniba-blue focus:ring-opacity-20 transition-all duration-200 outline-none"
                        placeholder="Otomatis dibuat dari nama jika kosong" value="{{ old('slug') }}">
                    <p class="mt-1 text-xs text-gray-500">Biarkan kosong untuk membuat slug otomatis dari nama</p>
                    @error('slug')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div> --}}
