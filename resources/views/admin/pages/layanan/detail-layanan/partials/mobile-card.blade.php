    ...
    @forelse ($items as $index => $item)
        <div
            ...
            <div class="p-4 space-y-3">
                ...
                <div>
                    <p class="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-1">Konten Preview</p>
                    <p class="text-sm text-gray-700 leading-relaxed line-clamp-3">
                        @php
                            $preview = '';
                            if (is_array($item->content)) {
                                foreach ($item->content as $section) {
                                    $preview .= ($section['title'] ?? '') . ' ' . ($section['content'] ?? '') . ' ';
                                }
                            }
                        @endphp
                        {{ Str::limit(strip_tags($preview), 150) ?: '-' }}
                    </p>
                </div>
            </div>

            <div class="grid grid-cols-3 gap-2 px-4 pb-4">
                <button @click="$dispatch('open-show-detail-layanan', {{ $item->toJson() }})"
                ...
                <button @click="$dispatch('open-edit-detail-layanan', {{ $item->toJson() }})"
                    class="inline-flex items-center justify-center gap-2 px-4 py-2.5
                ...
                <form method="POST" action="{{ route('admin.layanan.detail-layanan.destroy', $item) }}"
    ...
    @empty
        <div class="bg-white rounded-2xl shadow-md border border-gray-100 p-10 text-center">
            ...
            <p class="text-gray-400 text-sm mb-5 leading-relaxed">
                @if ($search)
                    Tidak ditemukan data detail layanan dengan kata kunci "<span
                        class="font-semibold text-gray-700">{{ $search }}</span>"
                @else
                    Belum ada data detail layanan yang ditambahkan
                @endif
            </p>
