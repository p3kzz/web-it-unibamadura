<div x-data="{ open: false, id: null, nama: '', deskripsi: '', icon: '', link: '', kategori_layanan_id: null, sort_order: 0, is_active: true }"
    x-on:open-edit-layanan.window="open = true; id = $event.detail.id; nama = $event.detail.nama; deskripsi = $event.detail.deskripsi; icon = $event.detail.icon; link = $event.detail.link; kategori_layanan_id = $event.detail.kategori_layanan_id; sort_order = $event.detail.sort_order; is_active = $event.detail.is_active"
    ...
    <div @click.outside="open = false"
        ...
        <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200">
            <h2 class="text-lg font-bold text-gray-800">Edit Layanan</h2>
        ...
        <form :action="`{{ url('admin_tik/katalog-layanan') }}/${id}`" method="POST" enctype="multipart/form-data"
            ...
            <div>
                ...
                <select name="kategori_layanan_id"
                    ...
                    @foreach ($categories as $cat)
                        <option value="{{ $cat->id }}" :selected="kategori_layanan_id == {{ $cat->id }}">
                            {{ $cat->nama }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div>
                ...
                <textarea name="deskripsi" rows="3" x-text="deskripsi"
                    class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-uniba-blue"></textarea>
            </div>
            ...
            <div>
                ...
                <template x-if="icon">
                    <img :src="`{{ asset('storage') }}/${icon}`" alt="icon saat ini"
                        class="w-12 h-12 object-cover rounded-lg border border-gray-200 mb-2">
                </template>
            ...
            <div>
                ...
                <input type="number" name="sort_order" :value="sort_order" min="0"
                    class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-uniba-blue">
            </div>

            <div class="flex items-center gap-3">
                <input type="hidden" name="is_active" value="0">
                <input type="checkbox" name="is_active" id="edit_is_active" value="1" :checked="is_active"
