...
@section('page-title', 'Detail Katalog Layanan')
@section('page-subtitle', 'Kelola informasi katalog layanan institusi')
...
@section('content')
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div x-data="{ open: false }" class="relative">
                <button @click="open = !open" @click.away="open = false"
                    class="inline-flex items-center justify-between gap-3 px-4 py-2.5 bg-white border-2 border-gray-300 rounded-lg hover:border-uniba-blue transition-all duration-200 shadow-sm hover:shadow-md min-w-[250px]">
                ...
                <div x-show="open" x-cloak x-transition:enter="transition ease-out duration-200"
                    ...
                    <a href="{{ route('admin.layanan.detail-layanan.index', array_filter(['search' => request('search')])) }}"
                        ...
                        <div class="flex-1">
                            <p class="text-sm font-semibold text-gray-800">Semua Katalog</p>
                        </div>
                    </a>
                    ...
                    @forelse ($katalogList as $katalog)
                        <a href="{{ route('admin.layanan.detail-layanan.index', array_filter(['katalog_layanan_id' => $katalog->id, 'search' => request('search')])) }}"
                            ...
                            <div class="flex-shrink-0">
                                @if (request('katalog_layanan_id') == $katalog->id)
                                    <div class="w-5 h-5 bg-blue-700 rounded-full flex items-center justify-center">
                                        <svg class="w-3 h-3 text-white" fill="none" stroke="currentColor"
                                            viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            <div class="flex-1">
                                <p class="text-sm font-semibold text-gray-800">{{ $katalog->nama }}</p>
                            </div>
                        </a>
                    @empty
                    @endforelse
            ...
            <button @click="$dispatch('open-create-detail-layanan')"
                ...
                <span class="font-semibold">
                    @if ($selectedKatalog)
                        Tambah Detail {{ $selectedKatalog->nama }}
                    @else
    ...
    <div id="ajax-table-container">
        @include('admin.pages.layanan.detail-layanan.partials.table')
    </div>

    @include('admin.pages.layanan.detail-layanan.partials.form-create')

    @include('admin.pages.layanan.detail-layanan.partials.form-edit')

    @include('admin.pages.layanan.detail-layanan.partials.form-show')
