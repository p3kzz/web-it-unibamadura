...
@section('page-title', 'Konfigurasi Footer')
@section('page-subtitle', 'Kelola konten footer situs web')
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div x-data="{ open: false }" class="relative">
                ...
                <div x-show="open" x-cloak x-transition:enter="transition ease-out duration-200"
                    ...
                    @foreach (['tautan cepat', 'sosial media'] as $t)
                        <a href="{{ route('admin.setting-footer.index', array_filter(['type' => $t, 'search' => $search])) }}"
            ...
            <button @click="$dispatch('open-create-konfigurasi-footer')"
                ...
                <span class="font-semibold">Tambah {{ ucfirst($type) }}</span>
            </button>
        </div>
    </div>

    <div id="ajax-table-container">
        @include('admin.pages.setting.konfigurasi-footer.partials.table')
    </div>

    @include('admin.pages.setting.konfigurasi-footer.partials.form-create')

    @include('admin.pages.setting.konfigurasi-footer.partials.form-edit')
