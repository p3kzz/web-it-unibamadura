<div x-data="{
    ...
    x-on:open-edit-prestasi.window="
        open = true;
        form = {
            id: $event.detail.id,
            ...
            achievement_date: $event.detail.achievement_date ? $event.detail.achievement_date.split('T')[0] : '',
    ...
    <div @click.away="open = false" x-transition:enter="transition ease-out duration-300"
        ...
        <div class="bg-uniba-blue px-6 py-4 flex items-center justify-between sticky top-0">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h3 class="text-xl font-bold text-white">Edit Prestasi</h3>
        ...
        <form :action="`/admin_tik/prestasi/${form.id}`" method="POST" enctype="multipart/form-data" class="p-6">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-5">
                <div>
                    ...
                    <input type="text" name="title" x-model="form.title"
                        ...
                        placeholder="Contoh: Best Digital Transformation Initiative" required>
                </div>

                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-2">
                        Pemberi Penghargaan <span class="text-red-500">*</span>
                    </label>
                    <input type="text" name="organization" x-model="form.organization"
                        class="w-full border-2 border-gray-300 rounded-lg px-4 py-2.5 focus:border-blue-500 focus:ring-2 focus:ring-blue-500 focus:ring-opacity-20 transition-all duration-200 outline-none"
                        placeholder="Contoh: Kemendikbudristek" required>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-bold text-gray-700 mb-2">
                            Tanggal Pencapaian <span class="text-red-500">*</span>
                        </label>
                        <input type="date" name="achievement_date" x-model="form.achievement_date"
                            class="w-full border-2 border-gray-300 rounded-lg px-4 py-2.5 focus:border-blue-500 focus:ring-2 focus:ring-blue-500 focus:ring-opacity-20 transition-all duration-200 outline-none"
                            required>
                    </div>
                ...
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-2">Foto/Sertifikat</label>
                    <template x-if="form.image">
                        <div class="mb-3">
                            <img :src="`/storage/${form.image}`" alt="Current image" class="max-w-xs rounded-lg border">
                            <p class="text-xs text-gray-500 mt-1">Gambar saat ini</p>
                        </div>
                    </template>
                    <input type="file" name="image" accept="image/*"
                        class="w-full border-2 border-gray-300 rounded-lg px-4 py-2.5 focus:border-blue-500 focus:ring-2 focus:ring-blue-500 focus:ring-opacity-20 transition-all duration-200 outline-none file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100">
                    <p class="mt-1 text-xs text-gray-500">Format: JPEG, PNG, JPG, WEBP. Maksimal 2MB. Biarkan kosong
                ...
                <div class="flex items-center gap-3">
                    ...
                    <label class="text-sm text-gray-700">Jadikan sebagai prestasi aktif</label>
                </div>
