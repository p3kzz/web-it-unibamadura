    ...
    @forelse ($items as $index => $item)
        <div
            ...
            <div class="p-4 space-y-3">
                <div>
                    <p class="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-1">Nama Tinjauan Manajemen</p>
                ...
                @if ($item->sections->count() > 0)
                    <div class="border-t border-gray-100 pt-3">
                        <p class="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-2">Bagian</p>
                        <div class="flex flex-wrap gap-1">
                            @foreach ($item->sections->take(3) as $section)
                                <span
                                    ...
                                    {{ Str::limit($section->title, 20) }}
                                </span>
                            @endforeach
                            @if ($item->sections->count() > 3)
                                <span class="text-xs text-gray-500 self-center">+{{ $item->sections->count() - 3 }}
                                    lainnya</span>
                            @endif
                        </div>
                    </div>
            ...
            <div class="grid grid-cols-3 gap-2 px-4 pb-4">
                <button @click="$dispatch('open-show-tinjauan-manajemen', {{ $item->toJson() }})"
                ...
                <button @click="$dispatch('open-edit-tinjauan-manajemen', {{ $item->toJson() }})"
                    class="inline-flex items-center justify-center gap-2 px-4 py-2.5
                ...
                <form method="POST" action="{{ route('admin.penjaminan.tinjauan-manajemen.destroy', $item) }}"
                    ...
                    <button type="submit" @disabled($item->is_active)
                        class="w-full inline-flex items-center justify-center gap-2 px-4 py-2.5
                            text-sm font-semibold rounded-xl shadow-sm transition-all duration-200
                            {{ $item->is_active
    ...
    @empty
        <div class="bg-white rounded-2xl shadow-md border border-gray-100 p-10 text-center">
            ...
            <p class="text-gray-700 font-semibold mb-1">
                ...
                @else
                    Belum ada data tinjauan manajemen
                @endif
            </p>
            <p class="text-gray-400 text-sm mb-5 leading-relaxed">
                @if ($search)
                    Tidak ditemukan data tinjauan manajemen dengan kata kunci "<span
                        class="font-semibold text-gray-700">{{ $search }}</span>"
                @else
                    Belum ada data tinjauan manajemen yang ditambahkan
                @endif
            </p>

            <button @click="$dispatch('open-create-tinjauan-manajemen')"
