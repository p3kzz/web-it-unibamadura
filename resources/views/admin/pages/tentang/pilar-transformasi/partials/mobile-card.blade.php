    ...
    <div class="bg-white rounded-2xl shadow-md border border-gray-100 p-4 space-y-3">
        ...
        <div x-data="{ open: false }" class="relative">
            ...
            <div x-show="open" @click.outside="open = false" x-cloak
                ...
                <a href="{{ route('admin.tentang.pilar-transformasi.index', array_filter(['search' => $search])) }}"
                ...
                @foreach ($periodes as $periode)
                    <a href="{{ route('admin.tentang.pilar-transformasi.index', array_filter(['periode_id' => $periode->id, 'search' => $search])) }}"
                        class="flex items-center gap-3 px-4 py-3 hover:bg-blue-50 transition-colors duration-150
    ...
    @forelse ($items as $index => $item)
        <div
            ...
            <div class="grid grid-cols-3 gap-2 px-4 pb-4">
                <button @click="$dispatch('open-show-pilar', {{ $item->toJson() }})"
                ...
                <form method="POST" action="{{ route('admin.tentang.pilar-transformasi.destroy', $item) }}"
    ...
    @empty
        <div class="bg-white rounded-2xl shadow-md border border-gray-100 p-10 text-center">
            ...
            <p class="text-gray-400 text-sm mb-5 leading-relaxed">
                @if ($search)
                    Tidak ditemukan pilar transformasi dengan kata kunci "<span
                        class="font-semibold text-gray-700">{{ $search }}</span>"
                @else
                    Belum ada data pilar transformasi yang ditambahkan
                @endif
            </p>
