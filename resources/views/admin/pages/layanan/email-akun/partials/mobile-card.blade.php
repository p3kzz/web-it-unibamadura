    ...
    @forelse ($items as $index => $item)
        <div
            ...
            <div class="p-4 space-y-3">
                <div>
                    <p class="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-1">Nama Email Akun</p>
            ...
            <div class="grid grid-cols-3 gap-2 px-4 pb-4">
                <button @click="$dispatch('open-show-email-akun', {{ $item->toJson() }})"
                ...
                <button @click="$dispatch('open-edit-email-akun', {{ $item->toJson() }})"
                    class="inline-flex items-center justify-center gap-2 px-4 py-2.5
                ...
                <form method="POST" action="{{ route('admin.layanan.email-akun.destroy', $item) }}"
    ...
    @empty
        <div class="bg-white rounded-2xl shadow-md border border-gray-100 p-10 text-center">
            ...
            <p class="text-gray-400 text-sm mb-5 leading-relaxed">
                @if ($search)
                    Tidak ditemukan data email akun dengan kata kunci "<span
                        class="font-semibold text-gray-700">{{ $search }}</span>"
                @else
                    Belum ada data email akun yang ditambahkan
                @endif
            </p>
