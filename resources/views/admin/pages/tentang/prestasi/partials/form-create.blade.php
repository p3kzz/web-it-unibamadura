    ...
    <div @click.away="open = false" x-transition:enter="transition ease-out duration-300"
        ...
        <div class="bg-uniba-blue px-6 py-4 flex items-center justify-between sticky top-0">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h3 class="text-xl font-bold text-white">Tambah Prestasi</h3>
        ...
        <form method="POST" action="{{ route('admin.tentang.prestasi.store') }}" enctype="multipart/form-data"
            class="p-6">
            @csrf
            ...
            <div class="space-y-5">
                <div>
                    ...
                    <input type="text" name="title"
                        ...
                        value="{{ old('title') }}" placeholder="Contoh: Best Digital Transformation Initiative"
                        required>
                    @error('title')
                ...
                <div>
                    ...
                    <input type="text" name="organization"
                        class="w-full border-2 border-gray-300 rounded-lg px-4 py-2.5 focus:border-uniba-blue focus:ring-2 focus:ring-uniba-blue focus:ring-opacity-20 transition-all duration-200 outline-none"
                        value="{{ old('organization') }}" placeholder="Contoh: Kemendikbudristek" required>
                    @error('organization')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="date" name="achievement_date"
                            class="w-full border-2 border-gray-300 rounded-lg px-4 py-2.5 focus:border-uniba-blue focus:ring-2 focus:ring-uniba-blue focus:ring-opacity-20 transition-all duration-200 outline-none"
                            value="{{ old('achievement_date') }}" required>
