...
}" x-on:open-show-prestasi.window="
        open = true;
        item = $event.detail
    "
    ...
    <div @click.away="open = false" x-transition:enter="transition ease-out duration-300"
        ...
        <div class="bg-uniba-blue px-6 py-4 flex items-center justify-between sticky top-0">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h3 class="text-xl font-bold text-white">Detail Prestasi</h3>
                    <p class="text-blue-100 text-sm">Informasi lengkap prestasi</p>
                </div>
            </div>
        ...
        <div class="p-6 space-y-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-gray-500 mt-1" x-text="item.organization"></p>
                </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div class="bg-gray-50 rounded-xl p-4">
                    <p class="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-2">Tahun</p>
                    <p class="font-semibold text-gray-900 text-2xl" x-text="item.year"></p>
                </div>
                <div class="bg-gray-50 rounded-xl p-4">
                    <p class="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-2">Tanggal Pencapaian</p>
                    <p class="font-semibold text-gray-900"
                        x-text="item.achievement_date ? new Date(item.achievement_date).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' }) : '-'">
                    </p>
                </div>
            </div>
            ...
            <template x-if="item.image">
                <div>
                    <p class="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-2">Foto/Sertifikat</p>
                    <img :src="`/storage/${item.image}`" alt="Prestasi image"
                        class="rounded-xl max-w-full border border-gray-200">
                </div>
            </template>
        </div>
