...
@section('title', $license->name . ' - Lisensi Software UPT TIK UNIBA Madura')

@section('content')
    @php
        $renderTextBlock = function (?string $value): string {
        ...
        <div class="container mx-auto px-4 relative z-10">
            <nav class="inline-flex text-blue-100 hover:text-white transition-colors">
                <ol class="flex items-center space-x-2 text-blue-300 text-sm">
                    ...
                    <li><a href="{{ route('lisensi-software.index') }}" class="hover:text-white transition-colors">Lisensi
                            Software</a></li>
                    <li><svg class="w-3.5 h-3.5 opacity-50" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
                        </svg></li>
                    <li class="text-white font-medium truncate max-w-[180px]">{{ $license->name }}</li>
                </ol>
            </nav>
            ...
            <div class="max-w-4xl mx-auto text-center">
                ...
                <p class="text-blue-100 text-lg md:text-xl max-w-2xl mx-auto leading-relaxed animate-fade-in-up stagger-3">
                    {{ $license->short_description ?: 'Informasi lisensi software resmi untuk mendukung layanan institusi.' }}
                </p>
            </div>
        </div>
        ...
        <div class="container mx-auto px-4">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-4xl mx-auto">
                <div
                    ...
                    <div class="text-sm font-bold text-gray-900">{{ $license->sections->count() }} Bagian</div>
                    <div class="text-[10px] text-gray-400 font-medium mt-1 uppercase tracking-wide">Struktur Lisensi</div>
                </div>
                ...
                <div
                    ...
                    <div class="text-sm font-bold text-gray-900">Aktif</div>
                    <div class="text-[10px] text-gray-400 font-medium mt-1 uppercase tracking-wide">Status Lisensi</div>
                </div>
                ...
                <div
                    ...
                    <div class="text-sm font-bold text-gray-900">{{ $license->updated_at?->format('d M Y') }}</div>
                    <div class="text-[10px] text-gray-400 font-medium mt-1 uppercase tracking-wide">Pembaruan Data</div>
                </div>
            </div>
        </div>
    </section>
            ...
            <div class="max-w-5xl mx-auto space-y-6">
                <div class="scroll-animate">
                    <div class="flex items-center gap-4 mb-6">
                        ...
                        <div>
                            <h2 class="text-2xl font-bold text-gray-900">Bagian Lisensi</h2>
                            <p class="text-sm text-gray-500">Detail kebijakan dan ketentuan lisensi software</p>
                        </div>
                    </div>
                </div>

                @forelse ($license->sections as $index => $section)
                    <article class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden scroll-animate">
                        ...
                        <div class="p-8 md:p-10 space-y-5">
                            @forelse ($section->contents as $content)
                                <div class="rounded-2xl bg-blue-50/60 border border-blue-100 px-5 py-4">
                                    <div class="text-gray-700 leading-relaxed prose prose-sm max-w-none">
                                        {!! $renderTextBlock($content->content) !!}
                                    </div>
                                </div>
                            @empty
                                <p class="text-sm text-gray-500 italic">Konten belum tersedia.</p>
                            @endforelse
                        </div>
                    </article>
                @empty
                    <div class="bg-white border-2 border-dashed border-gray-200 rounded-3xl p-12 text-center">
                        <h3 class="font-bold text-xl text-gray-900 mb-2">Bagian lisensi belum tersedia</h3>
                        <p class="text-gray-500 max-w-md mx-auto">
                            Data lisensi ini belum memiliki rincian bagian. Silakan cek kembali beberapa saat lagi.
                        </p>
                    </div>
                @endforelse

                @if ($relatedLicenses->isNotEmpty())
                    <div class="scroll-animate pt-2">
                        <div class="flex items-center gap-4 mb-6">
                            <div class="w-1.5 h-10 bg-gradient-to-b from-uniba-blue to-blue-400 rounded-full"></div>
                            <div>
                                <h2 class="text-2xl font-bold text-gray-900">Lisensi Terkait</h2>
                                <p class="text-sm text-gray-500">Referensi lisensi software lain yang tersedia</p>
                            </div>
                        </div>

                        <div class="grid sm:grid-cols-2 md:grid-cols-3 gap-4">
                            @foreach ($relatedLicenses as $related)
                                <a href="{{ route('lisensi-software.show', $related->slug) }}"
