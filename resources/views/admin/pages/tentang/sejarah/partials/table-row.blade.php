    ...
    <td class="px-6 py-4 max-w-md">
        ...
        @if (strlen($item->title) > 80)
            <button @click="expanded = !expanded"
                ...
                <svg class="w-3 h-3 transition-transform" :class="expanded ? 'rotate-90' : ''" fill="none"
        ...
        <td class="px-6 py-4 whitespace-nowrap">
            <div class="flex flex-wrap gap-2">
                ...
                @forelse($extras as $extra)
                    <span class="px-2.5 py-1 bg-blue-100 text-uniba-blue font-bold rounded-lg text-sm">
                        {{ $extra }}
                    </span>
                @empty
                    <span class="px-2.5 py-1 bg-blue-100 text-uniba-blue font-bold rounded-lg text-sm">
    ...
    <td class="px-6 py-4">
        <div class="flex items-center justify-center gap-2">
            <button @click="$dispatch('open-edit-histories', {{ $item->toJson() }})" title="Edit data"
                class="inline-flex items-center gap-1 px-3 py-2 bg-orange-500 hover:bg-orange-700 text-white text-xs font-semibold rounded-lg shadow hover:shadow-lg transition-all transform hover:-translate-y-0.5">
            ...
            <form method="POST" action="{{ route('admin.tentang.histories.destroy', $item) }}">
                ...
                <button type="button" onclick="confirmDelete(this)"
                    class="px-3 py-2 text-xs font-semibold rounded-lg transition-colors shadow-sm
                    {{ $item->is_active
