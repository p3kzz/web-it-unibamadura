<div x-data="{ open: false }" x-on:open-create-konfigurasi-footer.window="open = true" x-show="open" x-cloak
    ...
    <div @click.away="open = false" x-transition:enter="transition ease-out duration-300"
        ...
        <form method="POST" action="{{ route('admin.setting-footer.store') }}" class="p-6">
            @csrf
            <input type="hidden" name="type" value="{{ $type }}">

            <div class="space-y-5">

                @if (in_array($type, ['tautan cepat', 'sosial media']))
                    <div>
                ...
                <div>
                    ...
                    <textarea name="url" rows="5"
                        ...
                        placeholder="Masukkan url {{ $type }} dengan detail..." required>{{ old('url', $item->url ?? '') }}</textarea>
                    <p class="mt-1 text-xs text-gray-500 flex items-center gap-1">
                        Tuliskan url dengan jelas dan detail
                    </p>
                    @error('url')
                        <p class="mt-1 text-sm text-red-600 flex items-center gap-1">
                            <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
